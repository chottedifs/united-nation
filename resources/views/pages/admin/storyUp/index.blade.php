        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    <h3>Story Top</h3>
                    <a href="{{ route('storyUp.create') }}" class="btn btn-outline-primary icon-left mt-3 mb-3">Add Story
                        Top</a>
                </div>
                        ...
                        <tbody>
                            @foreach ($stories as $story)
                                <tr>
                                    ...
                                    <td><img src="{{ $story->StoryUp->image_cover }}" alt="image" width="90"></td>
                                    <td><img src="{{ $story->StoryUp->image_box }}" alt="image" width="90"></td>
                                    <td>{{ Str::limit($story->StoryUp->description, 100) }}</td>
                                    <td>
                                        <div class="d-flex">
                                            <a href="{{ route('storyUp.edit', $story->id) }}"
                                                class="btn btn-outline-warning .icon-left me-2"><i
                                                    class="bi bi-pencil-square"></i></a>
                                            <form onsubmit="return confirm('Are you sure ?');"
                                                action="{{ route('storyUp.destroy', $story->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
