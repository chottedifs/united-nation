        ...
        <div class="page-title">
            <div class="row">
                <div class="col-12 col-md-6 order-md-1 order-last">
                    ...
                    <a href="{{ route('reportUn.create') }}" class="btn btn-outline-primary icon-left mt-3 mb-3">Add Report Un
                        Reform</a>
                </div>
                        ...
                        <tbody>
                            @foreach ($contents as $content)
                                <tr>
                                    ...
                                    <td><img src="{{ Storage::url($content->ReportUn->image_1) }}" alt="" style="width: 50px;">
                                    </td>
                                    <td>{{ Str::limit($content->ReportUn->content_2, 100) }}</td>
                                    <td><img src="{{ Storage::url($content->ReportUn->image_2) }}" alt="" style="width: 50px;">
                                    </td>
                                    <td>{{ Str::limit($content->ReportUn->content_3, 100) }}</td>
                                    <td><img src="{{ Storage::url($content->ReportUn->image_3) }}" alt="" style="width: 50px;">
                                    </td>
                                    <td>{{ Str::limit($content->ReportUn->content_4, 100) }}</td>
                                    <td><img src="{{ Storage::url($content->ReportUn->image_4) }}" alt="" style="width: 50px;">
                                    </td>
                                    <td>{{ Str::limit($content->ReportUn->content_5, 100) }}</td>
                                    <td><img src="{{ Storage::url($content->ReportUn->image_5) }}" alt="" style="width: 50px;">
                                    </td>
                                    <td>{{ Str::limit($content->ReportUn->content_6, 100) }}</td>
                                    <td>
                                        <div class="d-flex">
                                            <a href="{{ route('reportUn.edit', $content->id) }}"
                                                class="btn btn-outline-warning .icon-left me-2"><i
                                                    class="bi bi-pencil-square"></i></a>
                                            <form onsubmit="return confirm('Are you sure ?');"
                                                action="{{ route('reportUn.destroy', $content->id) }}" method="POST">
                                                @csrf
                                                @method('DELETE')
